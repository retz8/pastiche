...
2. Write the locked decisions as a spec at `_dev/docs/spec/<task-name>.md` with `grill-to-spec` skill (kebab-case, e.g. `_dev/docs/spec/skills-pastiche-setup.md`).
...
**Rule 4 — Per-phase kickoff.** Before starting any phase in TODO.md, run a phase-level `grill-me` and write a phase spec at `_dev/docs/spec/phase-<n>-<short-name>.md` locking decisions shared across the phase's tasks. Per-task grillings inherit from it.
