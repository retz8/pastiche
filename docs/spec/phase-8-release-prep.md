...
- **Root `.claude-plugin/marketplace.json`** — a Claude-Code-only distribution catalog, hand-authored, forced to the repo root by Claude Code convention (`/plugin marketplace add` looks there). Its plugin entry uses `source: "./dist/claude-code"` and carries **no `version` field** — the version is resolved from the generated `plugin.json` (see decision 3). Other platforms do not use marketplaces — when Codex/Gemini land they bring their own root-level distribution entry point (e.g. `gemini-extension.json`, `.codex-plugin/`), not an added entry in `marketplace.json`. There is no shared catalog.
