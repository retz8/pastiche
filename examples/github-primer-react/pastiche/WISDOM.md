...
- [Stack] In a horizontal Stack laying out text of differing font sizes on one line (e.g. a semibold name beside a smaller, lighter timestamp/byline), use `align="baseline"` rather than `align="center"` — centering mixed-size text reads as misaligned, while a shared baseline sits the glyphs on one line.
...
- [Stack] In a horizontal Stack pairing a leading icon/visual with single-line text (e.g. a status icon beside a label), use `align="baseline"` so the icon sits on the text baseline — `align="center"` optically floats a small icon against the text's cap height.
